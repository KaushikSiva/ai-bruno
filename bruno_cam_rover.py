...
SURV_PATH = os.path.join(ROOT, "bruno_surveillance")
if SURV_PATH not in sys.path:
    sys.path.insert(0, SURV_PATH)

from camera_shared import make_camera, read_or_reconnect  # type: ignore

try:
    from robot_motion import MecanumWrapper  # type: ignore
except Exception:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=["builtin", "external"], default=os.environ.get("CAM_MODE", "external"))
    ...
    parser.add_argument("--forward-speed", type=int, default=int(os.environ.get("BRUNO_ROVER_FORWARD_SPEED", "35")))
    parser.add_argument("--turn-speed", type=int, default=int(os.environ.get("BRUNO_ROVER_TURN_SPEED", "35")))
    parser.add_argument("--turn-rotation", type=float, default=float(os.environ.get("BRUNO_ROVER_TURN_ROT", "0.45")))
    parser.add_argument("--min-motion-area", type=int, default=int(os.environ.get("BRUNO_ROVER_MIN_MOTION_AREA", "1800")))
    parser.add_argument("--center-deadband", type=float, default=float(os.environ.get("BRUNO_ROVER_CENTER_DEADBAND", "0.20")))

    parser.add_argument("--risk-stop-thresh", type=float, default=float(os.environ.get("BRUNO_ROVER_RISK_STOP", "0.80")))
    parser.add_argument("--risk-caution-thresh", type=float, default=float(os.environ.get("BRUNO_ROVER_RISK_CAUTION", "0.60")))
    parser.add_argument("--confidence-min", type=float, default=float(os.environ.get("BRUNO_ROVER_CONFIDENCE_MIN", "0.45")))
    parser.add_argument("--stop-hold-s", type=float, default=float(os.environ.get("BRUNO_ROVER_STOP_HOLD_S", "0.8")))
    parser.add_argument("--uncertain-frame-limit", type=int, default=int(os.environ.get("BRUNO_ROVER_UNCERTAIN_FRAME_LIMIT", "10")))
    parser.add_argument("--search-switch-s", type=float, default=float(os.environ.get("BRUNO_ROVER_SEARCH_SWITCH_S", "1.5")))
    ...
    parser.set_defaults(use_vlm=(os.environ.get("BRUNO_USE_VLM", "1").strip().lower() in ("1", "true", "yes", "on")))
    parser.add_argument("--vlm-provider", choices=["auto", "local", "remote"], default=os.environ.get("BRUNO_VLM_PROVIDER", "auto"))
    parser.add_argument("--vlm-local-base", default=os.environ.get("BRUNO_VLM_LOCAL_BASE", "http://127.0.0.1:8080/v1"))
    parser.add_argument("--vlm-remote-base", default=os.environ.get("BRUNO_VLM_REMOTE_BASE", os.environ.get("BRUNO_VLM_API_BASE", "http://localhost:1234/v1")))
    parser.add_argument("--vlm-local-model", default=os.environ.get("BRUNO_VLM_LOCAL_MODEL", "gemma3"))
    parser.add_argument("--vlm-remote-model", default=os.environ.get("BRUNO_VLM_REMOTE_MODEL", os.environ.get("BRUNO_VLM_MODEL", "gemma3")))
    parser.add_argument("--vlm-api-key", default=os.environ.get("BRUNO_VLM_API_KEY", "lm-studio"))
    parser.add_argument("--vlm-local-timeout-ms", type=int, default=int(os.environ.get("BRUNO_VLM_LOCAL_TIMEOUT_MS", "800")))
    parser.add_argument("--vlm-remote-timeout-ms", type=int, default=int(os.environ.get("BRUNO_VLM_REMOTE_TIMEOUT_MS", "1600")))
    parser.add_argument("--vlm-max-latency-ms", type=int, default=int(os.environ.get("BRUNO_VLM_MAX_LATENCY_MS", "1200")))
    parser.add_argument("--vlm-min-confidence", type=float, default=float(os.environ.get("BRUNO_VLM_MIN_CONFIDENCE", "0.45")))
    parser.add_argument("--vlm-interval", type=float, default=float(os.environ.get("BRUNO_VLM_INTERVAL_SEC", "1.5")))
    parser.add_argument("--vlm-cooldown-s", type=float, default=float(os.environ.get("BRUNO_VLM_COOLDOWN_S", "5.0")))
