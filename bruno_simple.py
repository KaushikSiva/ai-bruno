...
class BrunoSimple:
    ...
    def handle_movement(self, movement_command):
        ...
        forward_speed = movement_command.get('forward_speed', 0)
        turn_direction = movement_command.get('turn_direction', 'STRAIGHT')
        turn_speed = movement_command.get('turn_speed', 0)
        
        # Log movement command for debugging
        self.logger.info(f"🎮 Movement: {action}, Distance: {distance_cm:.1f}cm, Speed: {forward_speed}%, Turn: {turn_direction} {turn_speed}%")
        ...
        if action == 'STOP':
            if self.approach_mode:
                ...
                # Celebration nod if head controller available
            ...
            if not self.approach_mode:
                ...
                self.logger.info(f"🚀 Starting approach - Distance: {distance_cm:.1f}cm")
        ...
        if self.movement_controller:
            ...
            # Log simulated movement when no hardware
            if action != 'STOP':
                self.logger.info(f"🎯 [SIMULATED] Would move: {action} - Forward: {forward_speed}%, Turn: {turn_direction} {turn_speed}%")
