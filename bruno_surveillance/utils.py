...
class Paths:
    ...
    def safe_write_text(path: Path, content: str):
        try:
            ...
            LOG.info(f'💾 Caption saved: {path}')
        except Exception as e:
            LOG.warning(f'⚠️  Failed to write file {path}: {e}')
...
def ensure_relative_paths() -> Paths:
    ...
    LOG.info(f'Working directory: {p.base}')
    LOG.info(f'Images → {p.gpt_images}')
    LOG.info(f'Logs   → {p.logs}')
    return p

paths = ensure_relative_paths()

# Re‑export helpers (optional convenience)
save_image_path = paths.save_image_path
sidecar_txt_path = paths.sidecar_txt_path
safe_write_text = paths.safe_write_text
