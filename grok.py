...
# coding: utf-8
"""
Bruno Intelligent Exploration (with Grok Vision support)
- Supports both OpenAI GPT Vision and xAI Grok Vision
- Select via VISION_PROVIDER in .env (values: "openai" or "grok")
"""
...
LOG = logging.getLogger("bruno.grok")
...
# =========================
# VisionClient (OpenAI or Grok)
# =========================
class VisionClient:
    def __init__(self, cfg: Dict):
        self.cfg = cfg
        self.provider = os.environ.get("VISION_PROVIDER", "grok").lower()
        self.enabled, self.last_photo_time, self.photo_count = False, time.time(), 0
        self.descriptions = []
        ...
        if self.provider == "openai":
            ...
            if self.api_key:
                ...
                LOG.warning("✗ XAI_API_KEY not found")
        else:
